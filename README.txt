...
Automatic backups using Loft Deploy
   ...
   database to the normal output path (but inside a folder called
   loft_dev_auto), for every N minutes of active use. This is not based on
   cron, but rather on elapsed time. If you set this to back up every 15
   minutes, it will only do so if you're activaly using the site. It's
   like poor man's cron, it looks to see if N minutes has elapsed since
   the last backup, and it does this on a page load. Thus, no use, no page
   load. The following settings needed to be added to settings.local.php.
