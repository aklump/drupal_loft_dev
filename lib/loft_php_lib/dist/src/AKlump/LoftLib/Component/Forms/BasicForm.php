...
abstract class BasicForm {
     ...
     */
    protected static function applyCallbacks(array $callbacks, array &$values)
    {
        $defaults = static::getFormSchemaData('default');
        ...
        foreach ($callbacks as $tree => $callback) {
            ...
            is_callable($callback) && ($after = $callback($before));
            if ($after !== $before) {
                $values[$tree] = $after;
            }
            continue;
    ...
    protected function _validate(&$subject, $schema, $parents = null)
    {
        try {
            if (is_array($subject)) {
                foreach ($subject as $key => $value) {
                    ...
                    if (!array_key_exists($key, $schema)) {
                        ...
                        throw new InvalidFormItemException("Invalid form item: @item");
                    }
                    $subject[$key] = $this->_validate($value, $schema[$key], $parents);
                }
                array_pop($parents);
            }
            else {
                if ($schema['required'] && empty($subject)) {
                    throw new MissingRequiredFormItemException("@item is a required field");
                }
                elseif ($subject && !empty($schema['regex']) && !preg_match($schema['regex'], $subject)) {
                    throw new InvalidFormItemValueException("The submitted value of @item is invalid");
                }

                if (!empty($schema['postValidate']) && is_callable($schema['postValidate'])) {
                    $subject = $schema['postValidate']($subject, $schema);
                }
                array_pop($parents);
            ...
            // Clean up the exception and insert the form item parents.
            $type = get_class($exception);
            $item = Arrays::flattenParents($parents);
            $message = str_replace('@item', $item, $exception->getMessage());
            if (!isset($this->getFormState()->errors[$item])) {
                $this->getFormState()->errors[$item] = array(
                    'exception' => new $type($message, $exception->getCode(), $exception),
                    'item'      => $item,
                );
            }
        }
